...
function CategoryFilterVisual({
  ...
  return (
    <Text style={styles.filterFallbackEmoji}>
      {getBookstoreCategoryFallbackIcon(section)}
    </Text>
  );
}
...
export default function BookstoreCategoryScreen() {
  ...
  const sectionSlug =
    getSingleParam(params.slug) ?? '';
  const requestedStoreId = getSingleParam(
    params.storeId,
  );
  const passedLabel = getSingleParam(
    params.label,
  );
  ...
  const [
    selectedSection,
    setSelectedSection,
  ] = useState<CatalogSection | null>(null);
  ...
  async function loadCategory() {
    try {
      ...
      if (!storeId) {
        const bookstores = await listStores({
  ...
  const childCategories = useMemo(() => {
    ...
    return [...selectedSection.children].sort(
      (first, second) => {
        if (
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedFilterKey === 'all') {
      products = getCatalogSectionProducts(
        selectedSection,
        true,
      );
    } else if (selectedFilterKey === 'offers') {
      products = getCatalogSectionOffers(
        selectedSection,
      );
    } else {
      const selectedChild = childCategories.find(
        (category) =>
          category.id === selectedFilterKey,
      );
  ...
  const currentStoreItemCount = cartItems.reduce(
  ...
  const amountRemaining = Math.max(
    minimumOrder - currentStoreSubtotal,
    0,
  );
  ...
  const orderProgress =
    minimumOrder <= 0
      ? currentStoreItemCount > 0
        ...
        : 0
      : Math.min(
          currentStoreSubtotal / minimumOrder,
          1,
        );
  ...
  const pageTitle =
    selectedSection.name || passedLabel || '';
