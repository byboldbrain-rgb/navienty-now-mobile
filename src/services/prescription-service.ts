...
function getValidatedUploadMimeType(
  ...
  try {
    ...
    if (
      ...
      throw new Error("اختر صورة للروشتة أو ملف PDF فقط.");
    }
  ...
  try {
    ...
    if (error instanceof PrivateUploadValidationError) {
      if (error.code === 'too_large') {
        throw new Error("حجم الروشتة أكبر من 8 ميجابايت.");
      }

      if (error.code === 'invalid_size') {
        throw new Error("تعذر التحقق من حجم الروشتة أو أن الملف فارغ.");
      }
    }
  ...
  try {
    ...
    if (error instanceof PrivateUploadValidationError) {
      ...
      if (error.code === 'content_type_mismatch') {
        throw new Error("محتوى ملف الروشتة لا يطابق نوعه. اختر صورة JPEG أو PNG أو WebP أو ملف PDF صالح.");
      }
    }
