...
export async function createServiceBooking(
  ...
  const booking = mapServiceBooking(
    data as ServiceBookingRow,
  );

  /**
   * Best-effort campaign attribution. Booking creation is never rolled back
   * or surfaced as failed because analytics is unavailable.
   */
  await trackRememberedNotificationConversion({
    type: 'service_booking_created',
    resourceId: booking.id,
    valueEgp: booking.packagePrice,
  });
